...
st.set_page_config(page_title="Wave & Prophet Scanner + Nandi Daude", layout="wide")
...
# --- 2) Batch selector (batch_size symbols per batch) ---
batch_size   = 100
...
# --- 4) Fetch historical OHLCV (throttle 0.8s) ---
@st.cache_data
...
def get_prophet_chart(df, ticker, wave2_date=None):
    ...
    fig.add_trace(go.Scatter(
        ...
        marker=dict(color='black', size=6), name='Actual',
    ...
    fig.update_layout(
        title=f"{ticker} Prophet Fit{'' if not wave2_date else ' · Wave 2 Marker'}",
        xaxis_title='Date', yaxis_title='Price',
        hovermode='x unified', height=350
    )
    return fig, fc
...
def get_outlier_chart(dfc, fc2, dsel, ticker):
    ...
    fig.add_trace(go.Scatter(
        ...
        line=dict(color='black'), marker=dict(size=5, color='black'),
        name='Actual',
        hovertemplate='Date: %{x|%Y-%m-%d}<br>Close: %{y:.2f}<extra></extra>'
    ))
    fig.add_trace(go.Scatter(
    ...
    fig.add_trace(go.Scatter(
        ...
        fill='toself', fillcolor='rgba(173,216,230,0.3)',
        line=dict(color='rgba(255,255,255,0)'), showlegend=True, name='95% Interval',
        hoverinfo='skip'
    ))
...
# --- 10) Nandi Daude helper function ---
def find_nandi_trigger(df):
    df = df.copy()
    len_sma = 20
    mult = 2.0
    cmo_len = 9
    ...
    df["sma"]    = df["Close"].rolling(len_sma).mean()
    df["dev"]    = mult * (df["Close"] - df["sma"]).abs().rolling(len_sma).std()
    df["cmo"]    = ta.cmo(df["Close"], length=cmo_len)
    df["vol_sma"]= df["Volume"].rolling(len_sma).mean()

    # look only at last 6 days
    for i in range(len(df)-6, len(df)-1):
        if i <= 1:
            continue
        abs_kri      = abs(df["Close"].iloc[i] - df["sma"].iloc[i])
        abs_kri_prev = abs(df["Close"].iloc[i-1] - df["sma"].iloc[i-1])
        dev          = df["dev"].iloc[i]
        dev_prev     = df["dev"].iloc[i-1]
        change_perc  = ((df["Close"].iloc[i] - df["Close"].iloc[i-1]) / df["Close"].iloc[i-1]) * 100
        cmo_now      = df["cmo"].iloc[i]
        cmo_prev     = df["cmo"].iloc[i-1]

        condition1 = abs_kri > dev and abs_kri_prev <= dev_prev and change_perc >= 0
        condition2 = cmo_now > 0 and cmo_prev < 0

        if condition1 and condition2:
            white_high = df["High"].iloc[i]
            white_low  = df["Low"].iloc[i]
            # search for trigger within next 3 candles
            for j in range(i+1, min(i+4, len(df))):
                close = df["Close"].iloc[j]
                vol   = df["Volume"].iloc[j]
                if close > white_high and vol > df["vol_sma"].iloc[j] * vol_multiplier:
                    return i, j, {
                        "WhiteHigh":     white_high,
                        "WhiteLow":      white_low,
                        "TriggerClose":  close,
                        "TriggerVolume": vol
                    }
            return i, None, {"WhiteHigh": white_high, "WhiteLow": white_low}
    return None, None, {}
        ...
        for i, sym in enumerate(symbols, start=1):
            try:
                ...
                if df.empty or 'Close' not in df:
                    st.warning(f"No valid data for {sym}. Skipping.")
...
with tab2:
    ...
    if ticker != "-- Select --" and st.button("▶️ Generate forecast", key="p4"):
        ...
        if df_raw.empty:
            ...
            dfp = df_raw[['Date','Close']].rename(columns={'Date':'ds','Close':'y'}).dropna()
            if len(dfp) < 2:
...
with tab3:
    ...
    o_start = st.date_input("History start",
                            datetime.date.today() - datetime.timedelta(days=90),
                            key="o1")
    o_end   = st.date_input("History end",
                            datetime.date.today(),
                            key="o2")
    if st.button("▶️ Find outliers", key="o3"):
        ...
        for i, sym in enumerate(symbols, start=1):
            ...
            dfp2 = df[['ds','Close']].rename(columns={'ds':'ds','Close':'y'}).dropna()
            if len(dfp2) < 2:
                prog.progress(i/len(symbols))
                continue
            ...
            merged = pd.merge(
                df[['ds','Close']],
                fc2[['ds','yhat_lower','yhat_upper']],
                on='ds', how='inner'
            )
            bad = merged[(merged.Close < merged.yhat_lower) |
        ...
        if not outliers:
            ...
            for row in df_latest.itertuples(index=False):
                ...
                dfp2 = dfc[['ds','Close']].rename(columns={'ds':'ds','Close':'y'}).dropna()
...
# --- Tab 4: Nandi Daude ---
with tab4:
    st.header("🚀 Nandi Daude (White Candle + Trigger) Scanner")
    nd_start = st.date_input("Start date",
                             datetime.date.today() - datetime.timedelta(days=60),
                             key="n1")
    nd_end   = st.date_input("End date",
                             datetime.date.today(),
                             key="n2")

    if st.button("▶️ Scan Nandi Setup", key="n3"):
        results_nandi = []
        prog = st.progress(0)
        for i, sym in enumerate(symbols, start=1):
            try:
                df = fetch_price_data(token_map[sym], nd_start, nd_end)
                time.sleep(0.8)
                if df.empty or len(df) < 25:
                    prog.progress(i/len(symbols))
                    continue

                white_idx, nandi_idx, info = find_nandi_trigger(df)
                if white_idx is not None and nandi_idx is not None:
                    results_nandi.append({
                        "Ticker": sym,
                        "WhiteCandle": df["Date"].iloc[white_idx].date(),
                        "TriggerCandle": df["Date"].iloc[nandi_idx].date(),
                        **info
                    })
            except Exception as e:
        ...
        if results_nandi:
            df_nandi = pd.DataFrame(results_nandi)
            st.dataframe(df_nandi, use_container_width=True)
            st.download_button(
                "📥 Download Nandi CSV",
                to_csv_bytes(df_nandi),
                "nandi_stocks.csv",
                "text/csv"
            )
        else:
            st.info("No Nandi triggers found.")

# --- Tab 5: White Candles Only ---
with tab5:
    st.header("⚪ White Candle Scanner (No Trigger)")
    wc_start = st.date_input("Start date",
                             datetime.date.today() - datetime.timedelta(days=60),
                             key="wco1")
    wc_end   = st.date_input("End date",
                             datetime.date.today(),
                             key="wco2")

    if st.button("▶️ Scan White Candles Only", key="wco3"):
        results_white = []
        prog = st.progress(0)
        for i, sym in enumerate(symbols, start=1):
            try:
                df = fetch_price_data(token_map[sym], wc_start, wc_end)
                time.sleep(0.8)
                if df.empty or len(df) < 25:
                    prog.progress(i/len(symbols))
                    continue
                ...
                white_idx, nandi_idx, info = find_nandi_trigger(df)
                if white_idx is not None and nandi_idx is None:
        ...
        if results_white:
            df_white = pd.DataFrame(results_white)
            st.dataframe(df_white, use_container_width=True)
            st.download_button(
                "📥 Download White Candle CSV",
                to_csv_bytes(df_white),
                "white_candles.csv",
                "text/csv"
            )
        else:
            st.info("No white candles found.")
